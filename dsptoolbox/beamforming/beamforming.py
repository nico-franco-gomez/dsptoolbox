...
class SteeringVector:
    """This class hosts the main equation
    to be used for the steering vector."""

    # ======== Constructor ====================================================
    def __init__(self, formulation: str | callable = "true location"):
...
class BeamformerGridded(BaseBeamformer):
    ...
    def __init__(
        ...
        assert (
            isinstance(steering_vector) == SteeringVector
...
def mix_sources_on_array(
    ...
    if isinstance(sources, MonopoleSource):
        sources = [sources]
    assert (
        len(sources) > 0
    ), "There must be at least one source to project on array"
    assert all(
        [isinstance(i, MonopoleSource) for i in sources]
